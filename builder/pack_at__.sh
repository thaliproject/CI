...
ERROR_ABORT() {
  if [[ $? != 0 ]]
  then
    LOG $RED_COLOR "pack android tests aborted\n"
    exit -1
  fi
}
...
cp {{BUILD_PATH}} androidTest-unaligned.apk;ERROR_ABORT
zipalign -v 4 androidTest-unaligned.apk android.apk;ERROR_ABORT
