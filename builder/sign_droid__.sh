...
ERROR_ABORT() {
  if [[ $? != 0 ]]
  then
    LOG $RED_COLOR "sign android aborted\n"
    exit -1
  fi
}
### END - JXcore Test Server   --------

scp my-release-key.keystore thali@192.168.1.20:~/Github/testBuild/;ERROR_ABORT
ssh thali@192.168.1.20 'bash -s' < pack_android.sh;ERROR_ABORT
