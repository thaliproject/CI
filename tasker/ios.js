    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (str.indexOf('Found ') >= 0) {
        ...
        if (n1 >= 0) {
          ...
          var deviceId = str
            .substr(n1 + 1, str.length - (n1 + 1))
            .trim()
            .replace('(', '')
            .replace(')', '');

          logger.info('ios: device name: ' + name, ', device identifier: ', deviceId);
          if (deviceId.indexOf('\'') !== -1) {
            logger.error('ios: unexpected device identifier ', deviceId);
          }
...
var uninstallApp = function (job, cb) {
  ...
  function callback(err, stdout, stderr) {
    ...
    if (err && err.code != 253) {
      counter = -1;
      logger.error('Error: uninstalling app from device.', err, stdout, stderr);
      cb({"server" : [err + "\n\n" + stdout + stderr + "\n\n**Call Log**  \n" + call_log.join("\n") + "\n"]});
      return;
    }
  ...
  this.run = function () {
    ...
    child.on('exit', function (code) {
      ...
      if (!_this.killing) {
        ...
        logger.error('ios: child process exited with code ' + code, "on device", _this.deviceId);
      }
...
var test_ = function (job, cb) {
  ...
  IsFailed = false;
  // get devices
  logger.info('Getting the list of iOS devices');
  getIOSDevices(function (err) {
    ...
    logger.info('Deploying iOS test app')
